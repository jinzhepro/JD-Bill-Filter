...
export function CanteenPurchaseOrderManager() {
    ...
    try {
      ...
      tables.forEach((table, tableIndex) => {
        ...
        for (let rowIndex = 2; rowIndex < rows.length; rowIndex++) {
          ...
          const amountWithTax = new Decimal(totalAmountWithoutTax)
            .plus(taxAmount)
            .toNumber();
  ...
  const handleFileUpload = async (event) => {
    ...
    if (!file.name.endsWith(".docx") && !file.name.endsWith(".doc")) {
      toast({
        title: "请上传 .docx 或 .doc 格式的 Word 文件",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleDeleteBatch = async (batchNo) => {
    ...
    try {
      const res = await fetch(
        `/api/canteen-purchase-orders?batch_no=${batchNo}`,
        { method: "DELETE" },
      );
      const data = await res.json();
    ...
    const values = items
      .map((item) => {
        ...
        if (columnKey === "quantity") {
          return item.remaining_quantity != null
                    ...
                    <div className="flex justify-between items-center">
                      <div>
                        <CardTitle className="text-lg flex items-center gap-2">
                          发票号码: {batchNo}
                          <button
                            onClick={() => copyContractNo(batchNo, "发票号码")}
                            className="cursor-pointer p-0.5 hover:bg-muted rounded text-muted-foreground hover:text-foreground"
                            title="复制发票号码"
                          >
                            <Copy className="w-3 h-3" />
                        ...
                        <div className="flex gap-4 text-sm text-muted-foreground">
                          ...
                          <span className="flex items-center gap-1">
                            ...
                            <button
                              onClick={() =>
                                copyContractNo("JK-GQ-250041", "销售合同号")
                              }
                              className="cursor-pointer p-0.5 hover:bg-muted rounded text-muted-foreground hover:text-foreground"
                              title="复制销售合同号"
                            >
                        ...
                        <p className="text-sm text-muted-foreground">
                          ...
                          {formatAmount(
                            items
                              .reduce(
                                (sum, item) => sum.plus(item.total_amount || 0),
                                new Decimal(0),
                              )
                              .toNumber(),
                          )}{" "}
                          ...
                          {formatAmount(
                            items
                              .reduce(
                                (sum, item) => sum.plus(item.tax_amount || 0),
                                new Decimal(0),
                              )
                              .toNumber(),
                          )}{" "}
                      ...
                      <div className="flex items-center gap-2">
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => downloadCSV(batchNo, items)}
                        >
      ...
      <Dialog open={importModalOpen} onOpenChange={handleCloseImportModal}>
        <DialogContent className="max-w-4xl">
          ...
          <div className="space-y-4">
              ...
              >
                ...
                {suppliers.map((supplier) => (
                  <option key={supplier.id} value={supplier.id}>
                    {supplier.name}{" "}
                    {supplier.contract_no ? `(${supplier.contract_no})` : ""}
                  </option>
                ))}
              </select>
            </div>
            ...
            <p className="text-sm text-muted-foreground">
              支持 .docx 格式的 Word
              文件，文件中需包含表格。表格列顺序：项目名称、规格型号、单位、数量、单价、金额、税率/征收率、税额。规格型号和单位可为空或合并为一列。以「*」开头的行才会被解析。
            </p>

            {previewErrors.length > 0 && (
              <div className="bg-destructive/10 border border-destructive/20 rounded-lg p-3">
                <p className="text-sm text-destructive">
                  解析问题：{previewErrors.slice(0, 5).join("；")}
